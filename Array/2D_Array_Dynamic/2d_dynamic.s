.equ NULL, 0
.section .rodata
    msg_main_print_value_of_rows_column:
    .string "Enter the vales of rows and column\n"

    msg_main_scan_value_of_rows_column:
    .string "%d%d"

    msg_main_print_enter_value:
    .string "Enter arr[%d][%d]\t"

    msg_main_scan_value:
    .string "%d"

    msg_main_print_entered_value:
    .string "Entered value are\n"

    msg_main_print_value:
    .string "iArr[%d][%d]= %d\t"

    msg_main_print_malloc_failed:
    .string "Memory Allocation Failed\n" 

    msg_main_print_address_1d:
    .string "%d\n"   
.section .text
.globl  main 
.type   main,@function
main:
    pushl   %ebp
    movl    %esp,%ebp

    subl    $20,%esp

    movl    $NULL,-20(%esp)

    pushl   $msg_main_print_value_of_rows_column
    call    printf
    addl    $4,%esp

    leal    -4(%ebp),%eax
    leal    -8(%ebp),%edx
    pushl   %edx
    pushl   %eax
    pushl   $msg_main_scan_value_of_rows_column
    call    scanf
    addl    $12,%esp

    movl    -4(%ebp),%eax
    movl    $4,%ecx
    mull    %ecx
    pushl   %eax
    call    malloc
    addl    $4,%esp

    movl    %eax,-20(%ebp)

    cmpl    $NULL,%eax
    je      label_malloc_failed


    movl    $0,-12(%ebp)
    jmp     label_memory_allocation_1d_cond
label_memory_allocation_1d_statement:
    movl    -8(%ebp),%eax
    movl    $4,%ecx
    mull    %ecx
    pushl   %eax
    call    malloc
    addl    $4,%esp

    movl    %eax,%edx

    movl    -20(%ebp),%ebx
    movl    -12(%ebp),%eax
    movl    %edx,(%ebx,%eax,4)

    cmpl    $NULL,%edx
    je      label_malloc_failed

    addl    $1,-12(%ebp)
label_memory_allocation_1d_cond:
    movl    -12(%ebp),%eax
    movl    -4(%ebp),%edx
    cmpl    %edx,%eax
    jl      label_memory_allocation_1d_statement
     
    movl    $0,-12(%ebp)
    jmp     label_for_scan_row_condition 
label_for_scan_row_statement:
    movl    $0,-16(%ebp)
    jmp     label_for_scan_column_condition
label_for_scan_column_statement:
    movl    -12(%ebp),%edx
    pushl   %eax
    pushl   %edx
    pushl   $msg_main_print_enter_value
    call    printf
    addl    $12,%esp

    movl    -20(%ebp),%ebx
    movl    -12(%ebp),%eax
    movl    (%ebx,%eax,4),%ebx
    movl    -16(%ebp),%eax
    leal    (%ebx,%eax,4),%ebx
    pushl   %ebx
    pushl   $msg_main_scan_value
    call    scanf
    addl    $8,%esp

    addl    $1,-16(%ebp)

label_for_scan_column_condition:
    movl    -16(%ebp),%eax
    movl    -8(%ebp),%edx
    cmpl    %edx,%eax
    jl       label_for_scan_column_statement

    addl    $1,-12(%ebp)          

label_for_scan_row_condition:
    movl    -12(%ebp),%eax
    movl    -4(%ebp),%edx
    cmpl    %edx,%eax
    jl      label_for_scan_row_statement


    pushl   $msg_main_print_entered_value
    call    printf
    addl    $4,%esp

    movl    $0,-12(%ebp)
    jmp     label_for_print_row_condition 
label_for_print_row_statement:
    movl    $0,-16(%ebp)
    jmp     label_for_print_column_condition
label_for_print_column_statement:
    
    movl    -20(%ebp),%ebx
    movl    -12(%ebp),%edx
    movl    (%ebx,%edx,4),%ebx
    movl    -16(%ebp),%eax
    movl    (%ebx,%eax,4),%ebx
    pushl   %ebx
    pushl   %eax
    pushl   %edx
    pushl   $msg_main_print_value
    call    printf
    addl    $16,%esp

    addl    $1,-16(%ebp)

label_for_print_column_condition:
    movl    -16(%ebp),%eax
    movl    -8(%ebp),%edx
    cmpl    %edx,%eax
    jl       label_for_print_column_statement

    addl    $1,-12(%ebp)          

label_for_print_row_condition:
    movl    -12(%ebp),%eax
    movl    -4(%ebp),%edx
    cmpl    %edx,%eax
    jl      label_for_print_row_statement

    movl    $0,-12(%ebp)
    jmp     label_memory_deallocation_1d_cond
label_memory_deallocation_1d_statement:
    
    movl    -20(%ebp),%ebx
    movl    (%ebx,%eax,4),%ebx
    pushl   %ebx
    call    free
    addl    $4,%esp

    movl    $NULL,(%ebx,%eax,4)


    addl    $1,-12(%ebp)
label_memory_deallocation_1d_cond:
    movl    -12(%ebp),%eax
    movl    -4(%ebp),%edx
    cmpl    %edx,%eax
    jl      label_memory_deallocation_1d_statement 


    movl    -20(%ebp),%ebx
    pushl   %ebx
    call    free
    addl    $4,%esp

    movl    $NULL,-20(%ebp)


    pushl   $0
    call    exit


label_malloc_failed:
    pushl   $msg_main_print_malloc_failed
    call    printf
    addl    $4,%esp

    pushl   $-1
    call    exit    








    


    
    
    
    
    



